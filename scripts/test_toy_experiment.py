...
def _get_toy_destructors_and_names():
    ...
    for d in destructors:
        ...
        if 'max_canonical_destructors' in d.get_params():
            d.set_params(max_canonical_destructors=1, n_extend=None)
...
def test_toy_destructor(destructor_name):
    ...
    assert (np.abs(expected_train[ind] - result_dict['train_score'])
            / np.abs(expected_train[ind]) < 1e-15)
    assert (np.abs(expected_test[ind] - result_dict['test_score'])
            / np.abs(expected_test[ind]) < 1e-15)
